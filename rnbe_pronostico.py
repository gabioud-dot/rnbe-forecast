...
def fmt_tecnico(day_name: str, sem: str, temp: str, cond: str, slots, river: Optional[Dict]) -> str:
    L = []
    L += [f"### 🗓️ **{day_name}**", f"{sem}", "", f"🌡️ **Temperatura:** {temp}", f"☁️ **Condiciones:** {cond or '—'}", ""]
...
def main():
    ...
    # Fecha base = fecha del sistema (UTC en Actions) → próximo finde
    today = datetime.utcnow().date()
    sab, dom = next_weekend(today)

    # Verificar feriado lunes
    feriado = None
    monday = sab + timedelta(days=2)
    if monday.isoformat() in cfg.get("feriados", []):
        feriado = monday

    # Meteo: traer GFS/ICON y promediar
    models = []
    for m in cfg["models"]:
